...
public class CreepsGame {
	// The highest beaten battle tower. Player can be up to (battle tower * 5) + 5
	// level, beat tower 9 to win
	...
	public static void main(String[] args) {
		...
		while (playing) {
			...
			while (!(dir.equalsIgnoreCase("W") || dir.equalsIgnoreCase("A") || dir.equalsIgnoreCase("S")
					|| dir.equalsIgnoreCase("D"))) {
				...
				if (storage.size() > 0) {
					...
					for (int i = 0; i < storage.size(); i++)
						if (yN)
							...
							print(storage.get(i).name + " | " + storage.get(i).level + " "
									+ storage.get(i).rarString());
	...
	// Does the shop. The boolean tells if this is the first time checking the shop
	// (if this isn't shop() running shop())
	public static void shop(boolean orig) {
		if (orig) {
			if (world.world[row][col].metadata <= 0) // Reset stock if the user hasn't checked in for a while (>= 36
														// turns)
			{
		...
		if (world.world[row][col].forSale.size() > 0) {
			...
			for (int i = 0; i < world.world[row][col].forSale.size(); i++) {
				...
				println("PRICE FOR THIS CREEP: " + (world.world[row][col].forSale.get(i).level
						* world.world[row][col].forSale.get(i).rarity * 5 / 3) + "[S]");
				println("---------------------------------------------");
			}
			...
			if (shop > 0 && shop <= world.world[row][col].forSale.size()) {
				// Check price.
				if (shine >= world.world[row][col].forSale.get(shop - 1).level
						* world.world[row][col].forSale.get(shop - 1).rarity * 5 / 3) {
					println("Thank you for your purchase of this " + world.world[row][col].forSale.get(shop - 1).name
							+ ".");
					shine -= world.world[row][col].forSale.get(shop - 1).level
							* world.world[row][col].forSale.get(shop - 1).rarity * 5 / 3;
	...
	public static boolean doBattle(String loc, boolean wild) {
		...
		else if (!loc.equals("C") && !loc.equals("M")) // Not a battle tower (recognized as C for challenger or M for
														// Master)
			...
			while (true) {
				...
				if (pick >= 1 && pick <= 3) // Attack.
				{
					...
					if (creeps.get(0).attacks[pick - 1].exh <= creeps.get(0).eng) {
						...
						action = true;
						// Check for hit.
						if (creeps.get(0).attacks[pick - 1].spd > enemy.attacks[rn.nextInt(3)].spd
								|| rn.nextInt(3) == 0) {
				...
				{
					println("Your " + creeps.get(0).name + " backs up and rests. +"
							+ (creeps.get(0).maxENG / 6 + creeps.get(0).def) + " Energy!");
					nextLine();
					...
					action = true;
				} else if (pick == 6) // Swap creeps
				{
					if (creeps.size() >= 2) {
						println("What Creep will you swap to?");
						for (int i = 0; i < creeps.size(); i++)
							println((i + 1) + " : " + creeps.get(i).name + " | " + creeps.get(i).level + " "
									+ creeps.get(i).rarString());
						pick = userInt() - 1;
						if (pick < 0 || pick >= creeps.size())
							println("That number is invalid.");
						else if (pick != 0) {
							println("Swapping to Creep number " + (pick + 1) + ".");
							...
							println("You can't swap to the Creep that's already fighting!");
					} else
				...
				if (action) {
					...
					if (enemy.hp > 0) {
						if (enemy.eng > enemy.attacks[enmyAtk].exh) {
							...
							// Check for hit.
							if (enemy.attacks[enmyAtk].spd > creeps.get(0).attacks[rn.nextInt(3)].spd
									|| rn.nextInt(4) == 0) {
				...
				if (enemy.hp <= 0) {
					...
					if (wild) {
						...
						storage.add(enemy);
						println("You now have " + storage.size() + " creeps in storage and " + creeps.size()
								+ " with you.");
					}
	...
	public static String getSave() {
		return "$PLAYER/" + pLVL + "/" + pXP + "/" + shine + "/" + towerTop + "/C0<" + creeps.get(0).getSave() + ">/C1<"
				+ creeps.get(1).getSave() + ">/C2<" + creeps.get(2).getSave() + ">/";
	}
	...
	public static void AppendFile(String name, String toAdd) throws Exception// appends text to the given file, or
																				// creates it
	{
